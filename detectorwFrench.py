...
French-specific adjustments:
  French social media has heavier thread-posting culture (rapid-fire sequential
  tweets), producing more regular timing intervals in human accounts. Changes:
  - T2b CV: 10-11 post tier disabled (too many FPs); 12-14 tightened ≤0.80.
  - T3f spam: relaxed thresholds (sim>0.60, vocab<0.30) for French human
    spammers (betting tips, link-sharing accounts).
  - T1/T2 gating: multi-T3 bypass allows T3a(hashtag≥1) + T3d(opener≥2)
    to flag without T1/T2 support (catches French bots with human-like timing
    but content-level bot signatures; verified zero human FPs).
...
def _default_bots_for_dataset(dataset_path):
    ...
    # DS34/35 are submission datasets — no bots file
    return None
...
def tier2b_interval_regularity(times, lang="en"):
    ...
    Post-count tiers (English):
      10-11 posts: CV ≤ 1.10 → 2 pts
      12-14 posts: CV ≤ 0.90 → 4 pts
      ≥15 posts: full threshold range
    
    French adjustments: French thread-posting culture produces more regular
    intervals in normal human accounts — the bottom of the human CV
    distribution is much lower (0.57 vs 0.79+ for English).  Changes:
      10-11 posts: DISABLED (too many FPs at low post count)
      12-15 posts: tightened to CV ≤ 0.80 → 4 pts
      ≥16 posts: unchanged (thresholds still discriminative)
    ...
    if len(times) < 12:
        if is_fr:
            # French: skip 10-11 post tier entirely — too many FPs from
            # thread-posters and low-volume accounts with regular timing
            return 0, cv
        # English: ultra-strict threshold for 10-11 posts
        if cv <= 1.10:
            return 2, cv
        return 0, cv
    
    # French: extend the tighter 12-14 rule to also cover 15-post users.
    # At n=15, French human accounts (e.g. Lakers_World_Fr CV=1.019) still
    # have low CVs from thread-posting.  The ≥16 graduated thresholds are
    # discriminative enough at higher post counts.
    fr_small_limit = 16 if is_fr else 15
    if len(times) < fr_small_limit:
        # French: tighten from ≤0.90 to ≤0.80 (thread-posters have very low CV)
        threshold = 0.80 if is_fr else 0.90
        if cv <= threshold:
            return 4, cv
        return 0, cv
    
    # Standard thresholds for 15+ posts (English) / 16+ posts (French)
    if cv <= 0.8:
        return 5, cv
    elif cv <= 1.0:
        return 4, cv
...
def score_user(uid, posts, dataset_lang="en", username="", verbose=False):
    ...
    # French relaxed spam exemption: French human spammers (betting tips,
    # link-sharing accounts) have formulaic but slightly more varied content
    # that doesn't meet the strict English thresholds (sim>0.75, vocab<0.20).
    # Use relaxed thresholds (sim>0.60, vocab<0.30) to exempt them.
    if dataset_lang == "fr" and not detail and len(texts) >= 5:
        from difflib import SequenceMatcher as _SM
        _sim_sum, _sim_n = 0, 0
        for _i in range(len(texts) - 1):
            if len(texts[_i]) > 5 and len(texts[_i + 1]) > 5:
                _sim_sum += _SM(None, texts[_i], texts[_i + 1]).ratio()
                _sim_n += 1
        _avg_sim = _sim_sum / _sim_n if _sim_n > 0 else 0
        _all_words = ' '.join(texts).lower().split()
        _vocab = len(set(_all_words)) / len(_all_words) if _all_words else 1.0
        if _avg_sim > 0.60 and _vocab < 0.30:
            s, detail = -100, True
    ...
    # not reliable enough — EXCEPT when multiple strong T3 signals converge:
    # high hashtag density (T3a ≥ 1) AND repetitive openers (T3d ≥ 2)
    # together indicate a template bot even without timing/encoding signals.
    # Verified: zero humans across all 4 datasets have this T3 combination
    # without a T1/T2 signal.
    t1_total = sum(v["score"] for k, v in breakdown.items() if k.startswith("T1"))
    if t1_total == 0 and t2_total == 0:
        t3a_score = breakdown["T3a_hashtags"]["score"]
        t3d_score = breakdown["T3d_rep_opener"]["score"]
        if t3a_score >= 1 and t3d_score >= 2:
            pass  # Multi-T3 bypass: converging content signals are sufficient
        else:
    ...
    if ss_score > 0:
        ...
        # --- French thread-posting ratio rule ---
        # French users frequently post multi-tweet threads at the same second.
        # When same-second is the ONLY T2 signal and the ratio of same-second
        # posts to total posts is low (< 0.40), the same-second evidence is
        # better explained by occasional thread-posting than by bot behavior.
        # Bot same-second ratios are typically ≥ 0.44; human thread-posters
        # are ≤ 0.35. Verified: zero TP losses across all practice datasets.
        if dataset_lang == "fr" and corroborating <= 0:
            other_t2 = sum(v["score"] for k, v in breakdown.items()
                          if k.startswith("T2") and k != "T2a_same_second")
            ss_ratio = ss_count / n_posts if n_posts > 0 else 0
            if other_t2 <= 0 and ss_ratio < 0.40:
                total_score = min(total_score, 2)
...
def main():
    ...
    for dataset_path in dataset_paths:
        ...
        # Run detection
        detections, all_scores, ds_lang = detect_bots(
            dataset_path,
    ...
    if len(run_summaries) > 1:
        ...
        if eval_runs:
            ...
            for r in eval_runs:
                l = r.get("lang", "?")
                lang_groups.setdefault(l, []).append(r)

            if len(lang_groups) > 1:
                for l_code in sorted(lang_groups):
                    runs = lang_groups[l_code]
                    l_tp = sum(r["eval"]["tp"] for r in runs)
                    l_fp = sum(r["eval"]["fp"] for r in runs)
                    l_fn = sum(r["eval"]["fn"] for r in runs)
                    l_score = sum(r["eval"]["score"] for r in runs)
                    l_max = sum(r["eval"]["max"] for r in runs)
                    l_name = {"en": "English", "fr": "French"}.get(l_code, l_code)
                    print(
                        f"{l_name:10s} TP/FP/FN:  {l_tp} / {l_fp} / {l_fn}  "
                        f"Score: {l_score:+d} / {l_max} ({l_score / l_max:.0%})"
                    )
                print("-" * 60)
